...
<project>
  ...
  <dependencies>
    ...
    <!-- TODO: OpenRocket Core を組み込む場合はここに依存を追加 -->
  </dependencies>
        ...
        <executions>
          <execution>
            ...
            <configuration>
              ...
              <targetFileName>app.wasm</targetFileName>
...
TeaVM（WASM）では、`@Export` アノテーションを付与することで、Java のメソッドを WebAssembly のエクスポートとして公開できる。
...
`src/App.tsx` のイメージ（簡略版）:
> 現在の実装は `web-ui/src/App.tsx` を参照してください（入力フォーム、結果サマリ、グラフ、JS/Backend 比較などを含みます）。
...
> - `airDensityKgM3` を海面密度として扱い、`atmosphereScaleHeightM` を指定すると高度に応じて密度が指数減衰します（簡易モデル）。
> - 着地（高度 0m）に向けて負の高度へ突っ込まないよう、地面到達点を線形補間して最後の点として追加します。
...
```json
{
  ...
  "timeSeries": {
    ...
    "dragN": [0.0, 0.2, 0.9],
    "massKg": [0.35, 0.349, 0.348],
    "thrustN": [12.0, 12.0, 12.0]
  }
}
```
...
ブラウザで `http://localhost:5173` を開き、`Backend: wasm` / `Backend: stub` を確認して `Run Simulation` を押すと、入力フォーム・サマリ・グラフ（JS と Backend の比較）を確認できます。
